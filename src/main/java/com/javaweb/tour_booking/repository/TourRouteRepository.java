...
public interface TourRouteRepository extends JpaRepository<TourRoute, Long> {
    ...
    @Query(
        ...
        SELECT 
            ...
            tr.end_location,
            SUM(t.booked_seats) as total_booked,
            (SELECT t2.price FROM tour t2 WHERE t2.tour_route_id = tr.id AND t2.depature_date >= :sixMonthsAgo ORDER BY t2.depature_date DESC LIMIT 1) as latest_price,
            tr.image,
            (SELECT DATEDIFF(t2.return_date, t2.depature_date) + 1 FROM tour t2 WHERE t2.tour_route_id = tr.id AND t2.depature_date >= :sixMonthsAgo ORDER BY t2.depature_date DESC LIMIT 1) as duration_days,
            (SELECT GROUP_CONCAT(t3.depature_date ORDER BY t3.depature_date DESC SEPARATOR ',') FROM tour t3 WHERE t3.tour_route_id = tr.id AND t3.depature_date >= :sixMonthsAgo GROUP BY t3.tour_route_id LIMIT 1) as recent_start_dates
        FROM tour_route tr
